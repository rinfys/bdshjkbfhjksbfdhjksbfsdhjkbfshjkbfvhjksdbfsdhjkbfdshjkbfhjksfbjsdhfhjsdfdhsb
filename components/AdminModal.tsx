...
const AdminModal: React.FC<AdminModalProps> = ({ isOpen, onClose, players }) => {
    ...
    const handleUpdatePoints = async () => {
        if (!confirm('This will recalculate all player points based on match data (Server-Side). Continue?')) return;

        setIsUpdating(true);
        try {
            const response = await fetch('/api/update-points', { // Use relative path
                method: 'POST'
            });
